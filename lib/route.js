...
export class Route {
  ...
  /** @type {import("./route-group").RouteGroup<Component>} */ parent;
  ...
  getPage() {
    if (this.lazy) {
      return this.router.cache("lazy:route:" + this.fullPath, this.lazy);
    } else {
  ...
  getData(pathname, args) {
    if (this.load) {
      const key = `load:route:${pathname}?${args.searchParams.toString()}`;
