...
export class Router {
  ...
  action(actionFunction) {
    const actionTask = new Task(actionFunction);

    /** @type {import("./task").TaskCallback<Data>} */
    const actionCallback = (snapshot) => {
      Object.assign(action, snapshot);
      this.requestUpdate();
    };

    /** @type {Action<Args, Data>} */
    const action = (...args) => {
      const result = actionTask.run(args, actionCallback);
      actionCallback(actionTask.snapshot());
      return result;
    };
    ...
    action.invalidate = () => {
      actionTask.invalidate();
      Object.assign(action, actionTask.snapshot());
    };
 ...
 * @template [Data=unknown]
 * @typedef {ActionFunction<Args, import("./task").TaskSnapshot<Data>> & ActionStatic<Data> & import("./task").TaskSnapshot<Data>} Action
 */
